...
ВЫБРАТЬ
	...
	ВЫРАЗИТЬ(ЗначениеОбъем.Значение КАК Справочник.ЗначенияСвойствОбъектов).Представление   как item_volume
...
ИЗ
	Справочник.Номенклатура КАК СправочникНоменклатуры
		...
		ПО (ЗначенияДеливериСхем.Объект = СправочникНоменклатуры.Ссылка)
			...
			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияСвойствОбъектов КАК ЗначениеОбъем
		ПО (ЗначениеОбъем.Объект = СправочникНоменклатуры.Ссылка)
			И (ЗначениеОбъем.Свойство = &Объем)
...
ИЗ
	...
	Где  Год(ОтчетКомиссионераОПродажахТовары.Ссылка.Дата)=2023 и МЕСЯЦ(ОтчетКомиссионераОПродажахТовары.Ссылка.Дата)>5
"""

def get_query_sale_by_period():
    return """
ВЫБРАТЬ
	Продажи.ДокументПродажи.Номер КАК NumberReport,
	Продажи.ДокументПродажи.Дата КАК DateReport,
	Продажи.Номенклатура.Артикул КАК ArticulNom,
	Продажи.Номенклатура.Наименование КАК DescriptionNom,
	Продажи.Номенклатура.Код КАК CodeNom,
	ВЫБОР
		КОГДА Продажи.КоличествоОборот <> 0
			ТОГДА Продажи.СтоимостьОборот / Продажи.КоличествоОборот
		ИНАЧЕ 0
	КОНЕЦ КАК Price,
	Продажи.КоличествоОборот КАК Qty,
	Продажи.СтоимостьОборот КАК Sum,
	Продажи.ЗаказПокупателя.Номер КАК NumberOrder,
	Продажи.ЗаказПокупателя.Дата КАК DateOrder,
	Продажи.ДокументПродажи.Номер КАК NumberSale,
	Продажи.ДокументПродажи.Дата КАК DateSale,
	"" КАК IdNom,
	"" КАК Project,
	Продажи.Подразделение.Наименование КАК Department
ИЗ
	РегистрНакопления.Продажи.Обороты(&BeginPeriod, &EndPeriod, месяц, ) КАК Продажи
"""
